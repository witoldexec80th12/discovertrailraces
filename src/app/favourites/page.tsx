...
function extractDistance(id: string): string {
  const parts = id.split(/\s[–—-]\s/);
...
export default function FavouritesPage() {
  ...
  if (favourites.length === 0 && !isSearching) {
    return (
      <main className="min-h-screen bg-gradient-to-b from-neutral-50 to-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16">
          <div className="mb-8">
            <Link href="/cost" className="text-sm font-semibold text-neutral-500 hover:text-neutral-800 transition-colors">
              ← Cost Index
            </Link>
          </div>
          ...
          <div className="text-center py-24 rounded-2xl border border-dashed border-neutral-200 bg-white mt-6">
            ...
            <Link href="/cost" className="inline-block bg-neutral-900 text-white px-7 py-3 rounded-full text-sm font-semibold hover:bg-neutral-700 transition-colors">
              Browse races
            </Link>
          </div>
        </div>
              ...
              <ul className="space-y-3">
                {searchResults.map((r) => {
                  ...
                  return (
                    <li key={r.id}>
                      <Link
                        href={`/races/${slug}?from=/favourites&title=Saved+Races`}
                      ...
                      >
                        <div className="shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-neutral-100">
                          {imgUrl ? (
                            <img src={imgUrl} alt={name} className="w-full h-full object-cover group-hover:scale-[1.04] transition-transform duration-200" />
                          ) : (
                            <div className="w-full h-full bg-neutral-200" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-neutral-900 leading-snug line-clamp-2">{name}</p>
                        ...
                        {epk != null && (
                          <div className="shrink-0 text-right">
                            <p className="text-base font-bold text-neutral-900">€{epk.toFixed(2)}</p>
                  ...
                  >
                    <div className="flex items-start gap-0">
                      <div className="shrink-0 w-20 sm:w-24 flex flex-col items-center justify-center py-5 px-2 bg-[#1a2e4a] text-white text-center self-stretch">
                        {date ? (
                          <>
                            <span className="text-[11px] font-bold tracking-widest uppercase opacity-75">{date.month}</span>
                            <span className="text-3xl sm:text-4xl font-bold leading-none mt-0.5">{date.day}</span>
                            <span className="text-[11px] opacity-60 mt-1">{date.year}</span>
                          </>
                        ) : (
                      ...
                      <div className="flex-1 min-w-0 py-4 px-4 sm:px-5">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5">
                          {f.eurPerKm != null && (
                            <div>
                              ...
                              <span className="text-lg font-bold tabular-nums text-neutral-900">€{f.eurPerKm.toFixed(2)}</span>
                            </div>
                          )}
                          {f.distanceKm != null && (
                            <div>
                              <span className="text-[9px] uppercase tracking-wider text-neutral-400 block">Distance</span>
                              <span className="text-base font-semibold tabular-nums text-neutral-700">{f.distanceKm} km</span>
                            </div>
                          )}
                          {f.elevationM != null && (
                            <div>
                              <span className="text-[9px] uppercase tracking-wider text-neutral-400 block">Elevation</span>
                              <span className="text-base font-semibold tabular-nums text-neutral-700">{Math.round(f.elevationM).toLocaleString()} m</span>
                            </div>
                          )}
                          {f.percentIncrease != null && (
                            <div>
                              <span className="text-[9px] uppercase tracking-wider text-neutral-400 block">D+/km</span>
                              <span className="text-base font-semibold tabular-nums text-neutral-700">{Math.abs(Math.round(f.percentIncrease))} m</span>
                            </div>
                          )}
                        </div>
                        {(hasLogistics || f.logistics) && (
                          <div className="mt-3 pt-3 border-t border-neutral-100 flex flex-wrap gap-x-4 gap-y-1">
                            ...
                            {f.logistics && (
                              <span className="text-xs text-neutral-500 leading-snug">{f.logistics}</span>
                            )}
                          </div>
                        )}
                      </div>
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        placeholder="Search all races by name…"
        className="w-full border border-neutral-300 rounded-xl pl-9 pr-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-neutral-400 shadow-sm"
      />
      {searching && (
